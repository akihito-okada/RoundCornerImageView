...
class RoundCornerConstraintLayout : ConstraintLayout {
    ...
    private fun init(attrs: AttributeSet?) {
        ...
        attrs?.also {
            val typedArray = context.obtainStyledAttributes(it, R.styleable.RoundCornerConstraintLayout)
            if (typedArray != null) {
                cornerRadius = typedArray.getDimensionPixelSize(R.styleable.RoundCornerConstraintLayout_cornerRadius, DEFAULT_CORNER_RADIUS.toInt()).toFloat()
                targetCorner = TargetCorner.fromOrdinal(typedArray.getInt(R.styleable.RoundCornerConstraintLayout_targetCorner, TargetCorner.NONE.ordinal))
                typedArray.recycle()
            }
        }
